...
// send TCP request to server
	...
	tcp_seg.ack = 0;	// ack #
	tcp_seg.hdr_flags = flag_SYN(&tcp_seg);	// assign FYN
...
// receive ACK from server
	...
	tcp_seg.seq = generate_inital_seq() + 1;	// sequence number as initial client sequence number + 1 
...
// client responds back with an acknowledgement TCP segment
	...
	tcp_close.seq = generate_inital_seq2() + 1;	// Assign a sequence number as initial client sequence number + 1
